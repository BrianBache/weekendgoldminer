...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-[36px] left-0 right-0 z-50 bg-earth-800 dark:bg-dark-bg text-earth-50 shadow-lg" style={{ height: "70px" }}>
            ...
            <div key={item.label} className="border-b border-earth-700">
              {item.children ? (
                <>
                  <button
                    onClick={() => toggleDropdown(item.label)}
                    className="w-full text-left px-4 py-3 text-sm font-medium hover:bg-earth-700 dark:hover:bg-dark-surface flex justify-between items-center"
                  >
                    <span>
                      {item.label}
                      {item.locked && !isMember && <LockIcon />}
                ...
                <Link
                  href={item.href}
                  className="block px-4 py-3 text-sm font-medium hover:bg-earth-700 dark:hover:bg-dark-surface"
