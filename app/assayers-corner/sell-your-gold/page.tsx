  ...
  return (
    <div className="min-h-screen bg-earth-50 dark:bg-dark-bg">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        {/* Intro */}
        <div className="bg-white dark:bg-dark-surface rounded-lg shadow p-8 mb-12">
          <h2 className="text-2xl font-bold text-earth-800 dark:text-dark-text mb-4">Find Trusted Gold Buyers</h2>
          <p className="text-earth-600 dark:text-earth-300 text-lg mb-4">
        ...
        {/* Reputation System */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-earth-800 dark:text-dark-text mb-6">Understanding Our Reputation System</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {reputationTiers.map((tier) => (
              ...
              >
                ...
                <p className="text-earth-700 dark:text-earth-300">{tier.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Directory Structure */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-earth-800 dark:text-dark-text mb-6">Find Buyers by Location</h2>
          <div className="space-y-4">
            {statesData.map((stateData) => (
              <div key={stateData.state} className="bg-white dark:bg-dark-surface rounded-lg shadow-md overflow-hidden">
                <div className="bg-earth-100 dark:bg-dark-surface px-6 py-4 border-l-4 border-gold-500">
                  <h3 className="text-lg font-bold text-earth-800 dark:text-dark-text">{stateData.state}</h3>
                </div>
                <div className="p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {stateData.counties.map((county) => (
                      <button
                        key={county}
                        className="px-4 py-2 bg-earth-50 dark:bg-dark-bg border border-earth-300 dark:border-earth-700 rounded text-earth-700 dark:text-earth-300 hover:bg-gold-100 hover:border-gold-400 transition-colors font-medium text-sm"
                      >
                        {county}
                      </button>
                    ))}
                  </div>
                  <p className="text-earth-500 dark:text-earth-400 text-sm mt-4">
