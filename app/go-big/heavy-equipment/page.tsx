      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {auctions.map((auction) => (
            <div key={auction.name} className="bg-white dark:bg-dark-surface border-2 border-earth-300 dark:border-earth-700 rounded-lg p-8 hover:shadow-lg transition">
              <div className="h-20 bg-earth-200 dark:bg-earth-700 rounded mb-4 flex items-center justify-center">
                <span className="text-earth-600 dark:text-earth-300 font-semibold">{auction.name}</span>
              </div>
              <h3 className="text-xl font-bold text-earth-800 dark:text-dark-text mb-2">{auction.name}</h3>
              <p className="text-gold-600 text-sm font-semibold mb-3">{auction.type}</p>
              <p className="text-earth-600 dark:text-earth-300 mb-6">{auction.description}</p>
