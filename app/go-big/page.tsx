        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sections.map((section) => (
            <Link key={section.href} href={section.href}>
              <div className="bg-white border-2 border-earth-300 rounded-lg p-8 hover:shadow-lg hover:border-gold-400 transition cursor-pointer h-full">
                <div className="w-12 h-12 bg-gold-400 rounded-full mb-4"></div>
                <h2 className="text-2xl font-bold text-earth-800 mb-3">{section.title}</h2>
                <p className="text-earth-600 mb-4">{section.description}</p>
